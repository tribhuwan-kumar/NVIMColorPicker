...
class VIMColorPicker(customtkinter.CTkFrame):
    ...
    def on_key_press(self, event):
        ...
        self.canvas.create_image(self.target_x, self.target_y, image=self.target, tag="target")
        
        self.get_target_color()
        self.update_colors()


    def constrain_to_circle(self, x, y, circle_radius):

        distance = math.sqrt((x - circle_radius)**2 + (y - circle_radius)**2)

        if distance > circle_radius:
            angle = math.atan2(y - circle_radius, x - circle_radius)
    ...
    def bind_keys(self):
        self.master.bind("<KeyPress-h>", self.on_key_press)
        self.master.bind("<KeyPress-j>", self.on_key_press)
        self.master.bind("<KeyPress-k>", self.on_key_press)
        self.master.bind("<KeyPress-l>", self.on_key_press)

        self.master.bind("<KeyPress-O>", self.on_key_o)
        self.master.bind("<KeyPress-o>", self.on_key_O)


    def get_target_color(self):
        try:
            target_x = int(max(0, min(self.target_x, self.image_dimension -1)))
